...
from agent import SantaClausAgent

# Initialize the agent. You can choose between "openai" and "mistral" as the provider.
# Make sure to have OPENAI_API_KEY or MISTRAL_API_KEY in the Streamlit secrets
santa_claus_agent = SantaClausAgent(provider="openai")
...
if prompt := st.chat_input("All I want for Christmas is..."):
    ...
    with st.chat_message("assistant", avatar=avatars["assistant"]):
        ...
        streamed_response = santa_claus_agent.answer_and_log(
...
def _submit_feedback(feedback: dict):
    """
    This function is called when the user submits feedback.
    """
    # Map the feedback to the expected format
    map_feedback = {"👍": "success", "👎": "failure"}
    santa_claus_agent.feedback(
        flag=map_feedback.get(feedback["score"], None),
        notes=feedback["text"],
    )
...
if len(st.session_state.messages) > 1:
    feedback = streamlit_feedback(
        ...
        optional_text_label="(Optional) Some comments about your experience?",
